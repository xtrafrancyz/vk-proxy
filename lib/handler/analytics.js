...
setInterval(function() {
    ...
    console.info('['+Math.floor(mem.heapUsed/1e5)/10+'M/'+Math.floor(mem.heapTotal/1e5)/10+'M] Requests: ' + realtime.requests + '. Users online: ' + realtime.uniques.size + '. Users since start: ' + users.size);
    realtime.requests = 0;
...
module.exports.transform = function(response, req, res) {
    ...
    if (req.url == '/method/execute.getUserInfo') {
        ...
        if ('response' in json) {
            ...
            db.run('UPDATE users SET last_seen = ' + time + ' WHERE id = ' + profile['id']);
        }
    }
};
